...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        if (Arrays.asList(env.getActiveProfiles()).contains("test")) {
            http.headers().frameOptions().disable(); // permitir acesso ao h2
        }

        http.cors().and().csrf().disable(); // adiciona a config do cors e desabilita o CSRF
        ...
        http.authorizeRequests()
                ...
                .permitAll()
                .anyRequest()
                .authenticated(); // permitir qualquer requisição dos PUBLIC_MATCHERS
